app.controller('IndexController', ['$scope', '$http', '$filter', '$timeout', '$compile',
  function($scope, $http, $filter, $timeout, $compile) {
    ...
    $scope.test = function() {
          $scope.movie.ready = false;
      var v = $scope.voice,
          voice = '<voice ng-src="' + v.list[v.index] + '"   autoplay callback="test"></voice>',
          html = $compile(voice)($scope);
          $('.audio').append(html);
          v.index = ++v.index < v.list.length ? v.index : 0;
    }
    ...
    function start() {
        $scope.movie.ready= true,
        $scope.login.ready = false;
        $scope.movie.src = $scope.movie.list[3],
